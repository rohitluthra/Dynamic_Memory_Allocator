...
Test(sf_memsuite_student, second_malloc_is_over_four_pages, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	sf_malloc(56);
	void *x = sf_malloc(PAGE_SZ << 2);
	...
	cr_assert_null(x, "x is not NULL!");
	assert_free_block_count(0, 1);
	assert_free_block_count(16256, 1);
	cr_assert(sf_errno == ENOMEM, "sf_errno is not ENOMEM!");
}

Test(sf_memsuite_student, last_malloc_overflow, .init = sf_mem_init, .fini = sf_mem_fini) {
	int x =calculate (15, 68);
	//debug("%d",x);
	cr_assert(x==96, "x is not int value (16 multiple)!");
}

Test(sf_memsuite_student, getIndexCheck, .init = sf_mem_init, .fini = sf_mem_fini) {
	int x =getPositionForSF (68);
	//debug("%d",x);
	cr_assert(x==2, "x is valid index in range!");
}

Test(sf_memsuite_student, getIndexCheckneg, .init = sf_mem_init, .fini = sf_mem_fini) {
	int x =getPositionForSF (-1);
	//debug("%d",x);
	cr_assert(x==-1, "x is not valid index in range!");
}
